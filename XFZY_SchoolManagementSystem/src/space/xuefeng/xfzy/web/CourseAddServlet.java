...
//            throw new ServletException("Access Denied!");
        ...
        String receivedClientData = request.getParameter("courseToAdd");
        if(receivedClientData == null || receivedClientData.isEmpty())
        {
            throw new ServletException("Parameter courseToAdd is null");
        }

        Course courseToAdd = gson.fromJson(receivedClientData, Course.class);
        Connection dbConnection = null;
        try {
            dbConnection = dbUtil.getDatabaseConnection();
            boolean bSuccess = courseInfoDao.addNewCourse(dbConnection, courseToAdd);
