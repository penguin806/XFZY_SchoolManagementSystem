...
public class UserAddServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String receivedClientData = request.getParameter("userToAdd");
        if(receivedClientData == null || receivedClientData.isEmpty())
        {
            throw new ServletException("Parameter userToAdd is null");
        }

        User userToAdd = gson.fromJson(receivedClientData, User.class);
        Connection dbConnection = null;
        try {
            dbConnection = dbUtil.getDatabaseConnection();
            boolean bSuccess = userInfoDao.addNewUser(dbConnection, userToAdd);
