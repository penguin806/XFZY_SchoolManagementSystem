...
@WebServlet(name="ClassAddServlet", urlPatterns ={ "/addClass" })
...
//            throw new ServletException("Access Denied!");
        ...
        String receivedClientData = request.getParameter("classToAdd");
        if(receivedClientData == null || receivedClientData.isEmpty())
        {
            throw new ServletException("Parameter classToAdd is null");
        }

        Class classToAdd = gson.fromJson(receivedClientData, Class.class);
        Connection dbConnection = null;
        try {
            dbConnection = dbUtil.getDatabaseConnection();
            boolean bSuccess = classInfoDao.addNewClass(dbConnection, classToAdd);
